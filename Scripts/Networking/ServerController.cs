...
public class ServerController : MonoBehaviour
{
    ...
    private void Awake()
    {
        threadArr = new Thread[_serverCount];
        isStartArr = new bool[_serverCount];
        string ipAddress = UIController.ip;
        int _port = UIController.port;

        ServerThreadStart(ipAddress, _port, 0, "Main");
        ServerThreadStart(ipAddress, _port + 1, 1, "Transform");
        ServerThreadStart(ipAddress, _port + 2, 2, "Chat");
    ...
    private void ServerThreadStart(string ip, int port, int serverNum, string serverName)
    {
        IPAddress localAddr = IPAddress.Parse(ip);
        TcpListener tcpListener = new TcpListener(localAddr, port);
        serverList.Add(tcpListener);
        threadArr[serverNum] = new Thread(new ThreadStart(() =>
        {
            ServerStart(tcpListener, serverNum, client =>
            {
                if (serverName == "Main")
                    MainServer(client);
                if (serverName == "Chat")
        ...
        threadArr[serverNum].Start();
    }
    private void ServerStart(TcpListener server, int isStartIndex, Action<TcpClient> enterClient)
    {
        try
        {
            server.Start();
            isStartArr[isStartIndex] = true;
            while (true)
            {
    ...
    {   // WSACancelBlockingCall : 메인스레드에서 다른 스레드에서 작동하는 서버를 종료할 때 해당 에러 발생
